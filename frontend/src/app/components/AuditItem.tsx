...
import { ChevronDown, ChevronRight, AlertTriangle, XCircle, Info, Sparkles } from "lucide-react";
...
const getSeverityIcon = (score: number | null) => {
  if (score === null || score === undefined) return <Info className="w-4 h-4 text-gray-400 flex-shrink-0" />;
  if (score < 0.5) return <XCircle className="w-4 h-4 text-red-500 flex-shrink-0" />;
  if (score < 0.9) return <AlertTriangle className="w-4 h-4 text-yellow-500 flex-shrink-0" />;
          ...
          <thead>
            <tr className="bg-white/5">
              {headings.map((h: any, i: number) => (
                <th key={i} className="px-3 py-2 text-left text-xs font-semibold text-gray-400 uppercase tracking-wider whitespace-nowrap">
              ...
              <tr key={rowIdx} className="hover:bg-white/[0.02]">
                {headings.map((h: any, colIdx: number) => {
                  ...
                  if (typeof cellValue === 'object' && cellValue !== null) {
                    cellValue = cellValue.url || cellValue.text || JSON.stringify(cellValue);
                  }

                  // Format bytes
                  if (h.valueType === 'bytes' && typeof cellValue === 'number') {
                    cellValue = formatBytes(cellValue);
                  }
                  // Format ms
                  if (h.valueType === 'timespanMs' && typeof cellValue === 'number') {
                    cellValue = `${cellValue.toFixed(0)} ms`;
                  }

                  const isUrl = typeof cellValue === 'string' && (cellValue.startsWith('http') || cellValue.startsWith('/'));

                  return (
                    <td key={colIdx} className="px-3 py-2 text-gray-300 text-xs max-w-[300px]">
                      {isUrl ? (
                        <span className="text-blue-400 truncate block" title={cellValue}>{cellValue.length > 60 ? '...' + cellValue.slice(-57) : cellValue}</span>
                      ) : (
      ...
      <ul className="mt-3 space-y-1 text-sm text-gray-400">
        {details.items.slice(0, 5).map((item: any, i: number) => (
          <li key={i} className="pl-4 border-l-2 border-white/10">
            {typeof item === 'string' ? item : item.text || item.url || JSON.stringify(item)}
          </li>
        ))}
      </ul>
    );
...
export default function AuditItem({ audit, defaultExpanded = false }: AuditItemProps) {
  const [expanded, setExpanded] = useState(defaultExpanded);
  const hasDetails = audit.details_json && (audit.details_json.type === 'table' || audit.details_json.type === 'opportunity' || audit.details_json.type === 'list') && audit.details_json.items?.length > 0;

  return (
    <div className={`border-l-2 ${getSeverityBorder(audit.score)} transition-colors`}>
      <button
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            <h4 className="font-semibold text-slate-900 dark:text-gray-200 text-sm">{audit.title}</h4>
            {audit.display_value && (
          ...
          {(audit.savings_ms || audit.savings_bytes) && (
            <div className="flex gap-3 mt-1.5">
              {audit.savings_ms && audit.savings_ms > 0 && (
                <span className="text-xs font-mono text-purple-400">Est. savings: {Math.round(audit.savings_ms)} ms</span>
              )}
              {audit.savings_bytes && audit.savings_bytes > 0 && (
                <span className="text-xs font-mono text-purple-400">Est. savings: {formatBytes(audit.savings_bytes)}</span>
              )}
            </div>
          )}
        </div>
      ...
      <AnimatePresence>
        {expanded && hasDetails && (
          ...
          >
            <p className="text-xs text-gray-500 mb-2 leading-relaxed" dangerouslySetInnerHTML={{ __html: audit.description || '' }} />
