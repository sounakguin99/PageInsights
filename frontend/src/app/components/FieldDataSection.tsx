...
function FieldMetricCard({ metric }: { metric: FieldMetric }) {
  ...
  return (
    <div
      className={`p-4 rounded-xl border ${colors.border} bg-slate-50/50 dark:bg-white/[0.02]`}
    >
      <div className="flex items-center justify-between mb-2">
        <h4 className="text-xs uppercase font-semibold text-slate-500 dark:text-gray-400 tracking-wide">
          {metric.label}
        </h4>
        {metric.category && (
          <span
...
export default function FieldDataSection({ report }: FieldDataSectionProps) {
  const hasFieldData =
    report?.field_fcp_ms || report?.field_lcp_ms || report?.field_cls_score;
  if (!hasFieldData) return null;

  // Determine Core Web Vitals pass/fail: LCP, INP, CLS must all be "FAST" or "AVERAGE"
  const coreMetrics = [
    report.field_lcp_category,
    report.field_inp_category,
    report.field_cls_category,
  ];
  const coreWebVitalsPassed = coreMetrics.every(
    (c: string) => c && c !== "SLOW",
  );
  ...
  const metrics: FieldMetric[] = [
    {
      label: "FCP",
      value: report.field_fcp_ms,
      unit: "ms",
      category: report.field_fcp_category,
      distributions: report.field_fcp_distributions,
    },
    {
      label: "INP",
      value: report.field_inp_ms,
      unit: "ms",
      category: report.field_inp_category,
      distributions: report.field_inp_distributions,
    },
    {
      label: "LCP",
      value: report.field_lcp_ms,
      unit: "ms",
      category: report.field_lcp_category,
      distributions: report.field_lcp_distributions,
    },
    {
      label: "CLS",
      value: report.field_cls_score,
      unit: "",
      category: report.field_cls_category,
      distributions: report.field_cls_distributions,
    },
    {
      label: "TTFB",
      value: report.field_ttfb_ms,
      unit: "ms",
      category: report.field_ttfb_category,
      distributions: report.field_ttfb_distributions,
    },
  ];
    ...
    <div className="glass-card p-6 border border-slate-200 dark:border-white/10">
      <div className="flex items-center justify-between mb-1">
        <h3 className="text-xl font-bold flex items-center gap-2 text-slate-900 dark:text-white">
          <Globe className="text-blue-500 dark:text-blue-400" />
      ...
      <p className="text-slate-500 dark:text-gray-500 text-sm mb-6">
        Chrome User Experience Report (CrUX) — real-world field data collected
        over 28 days.
      </p>
