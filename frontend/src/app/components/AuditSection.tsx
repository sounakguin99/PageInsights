...
import { ChevronDown, ChevronUp, CheckCircle2, Zap, Info, AlertTriangle } from "lucide-react";
import AuditItem from "./AuditItem";

interface AuditSectionProps {
  title: string;
  audits: any[];
  type: 'opportunity' | 'diagnostic' | 'passed' | 'failing' | 'manual' | 'not-applicable';
...
const iconMap: Record<string, React.ReactNode> = {
  opportunity: <Zap className="w-5 h-5 text-yellow-500" />,
  diagnostic: <Info className="w-5 h-5 text-blue-400" />,
  passed: <CheckCircle2 className="w-5 h-5 text-green-500" />,
  failing: <AlertTriangle className="w-5 h-5 text-red-500" />,
  manual: <Info className="w-5 h-5 text-gray-400" />,
  'not-applicable': <Info className="w-5 h-5 text-gray-500" />,
};

const badgeColorMap: Record<string, string> = {
  opportunity: 'bg-yellow-500/10 text-yellow-500',
  diagnostic: 'bg-blue-500/10 text-blue-400',
...
export default function AuditSection({ title, audits, type, defaultCollapsed = false, icon, badgeColor }: AuditSectionProps) {
      ...
      return (
        <div className="glass-card border border-white/10 overflow-hidden">
          <div className="px-6 py-5 border-b border-white/5 bg-white/[0.03] flex items-center justify-between">
            ...
            <span className={`px-3 py-1 rounded-full text-xs font-bold ${badgeColor || badgeColorMap[type]}`}>0</span>
          </div>
      ...
      <div className="glass-card border border-white/10 dark:border-white/5 overflow-hidden">
        ...
        >
          <div className="flex items-center gap-3">
            {icon || iconMap[type]}
            <h3 className="text-base font-bold text-slate-800 dark:text-white"> {title} ({audits.length})</h3>
          </div>
              ...
              <div className="divide-y divide-white/5 max-h-[400px] overflow-y-auto">
                {audits.map((audit: any, idx: number) => (
                  <div key={idx} className="px-6 py-3 flex items-start gap-3">
                    ...
                    <h4 className="font-medium text-gray-400 text-sm">{audit.title}</h4>
                  </div>
                ))}
              </div>
            </motion.div>
  ...
  return (
    <div className="glass-card border border-white/10 dark:border-white/5 overflow-hidden">
      <div className="px-6 py-5 border-b border-slate-200 dark:border-white/5 bg-slate-50 dark:bg-white/[0.03] flex items-center justify-between">
        ...
        <span className={`px-3 py-1 rounded-full text-xs font-bold ${badgeColor || badgeColorMap[type]}`}>
          {audits.length}
        </span>
      </div>
